...
use crate::config::get_dynamo_table;
...
impl DynamoDbClient{
    ...
    pub async fn load_game(&self,player_id:&str)->Result<Option<GameState>,String>{
        ...
        let response=match self.client.get_item()
            .table_name(&self.table_name)
        ...
        match response.item{
            Some(item)=>{
                let game_data_attr = item.get("game_data")
                    .ok_or_else(||"Missing 'game_data' attribute".to_string())?;
                
                let game_json = game_data_attr.as_s()
